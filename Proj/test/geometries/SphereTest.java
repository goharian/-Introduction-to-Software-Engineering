...
public class SphereTest {
    ...
    public void testFindIntersections() {
        ...
        assertEquals("Ray's line out of sphere",
                null, sphere.findIntersections(new Ray(new Vector(1, 1, 0), new Point3D(-1, 0, 0))));
        ...
        result = sphere.findIntersections(new Ray(new Vector(0, 0, 2), new Point3D(1, 0, 0.5)));
        ...
        p = new Point3D(2, 0, 0);
        result = sphere.findIntersections(new Ray(new Vector(2, 0, -2), new Point3D(1, 0, 1)));
        assertEquals("Wrong number of points", 1, result.size());
        ...
        result = sphere.findIntersections(new Ray(new Vector(0, 0, 3), new Point3D(1, 0, -1.5)));
        assertEquals("Wrong number of points", 2, result.size());
        if (result.get(0).getZ().get() > result.get(1).getZ().get())
            result = List.of(result.get(1), result.get(0));
        assertEquals("Ray starts before the sphere", List.of(p1, p2), result);
        // TC14: Ray starts at sphere and goes inside (1 points)
        result = sphere.findIntersections(new Ray(new Vector(0, 0, 2.5), new Point3D(1, 0, -1)));
        assertEquals("Wrong number of points", 1, result.size());
        assertEquals("Ray starts at sphere and goes inside", p2, result.get(0));
        // TC15: Ray starts inside (1 points)
        result = sphere.findIntersections(new Ray(new Vector(0, 0, 2.5), new Point3D(1, 0, -0.5)));
        assertEquals("Wrong number of points", 1, result.size());
        assertEquals("Ray starts inside", p2, result.get(0));
        // TC16: Ray starts at the center (1 points)
        result = sphere.findIntersections(new Ray(new Vector(0, 0, 2.5), new Point3D(1, 0, 0)));
        assertEquals("Wrong number of points", 1, result.size());
        assertEquals("Ray starts at the center", p2, result.get(0));
        // TC17: Ray starts at sphere and goes outside (0 points)
        assertEquals("Ray starts at sphere and goes outside",
                null, sphere.findIntersections(new Ray(new Vector(0, 0, 2), new Point3D(1, 0, 1))));
        // TC18: Ray starts after sphere (0 points)
        assertEquals("Ray starts after sphere",
                null, sphere.findIntersections(new Ray(new Vector(0, 0, 2), new Point3D(1, 0, 2))));


        // **** Group: Ray's line is tangent to the sphere (all tests 0 points)
        // TC19: Ray starts before the tangent point
        assertEquals("Ray starts before the tangent point",
                null, sphere.findIntersections(new Ray(new Vector(2, 0, 0), new Point3D(0, 0, 1))));
        // TC20: Ray starts at the tangent point
        assertEquals("Ray starts at the tangent point",
                null, sphere.findIntersections(new Ray(new Vector(2, 0, 0), new Point3D(1, 0, 1))));
        // TC21: Ray starts after the tangent point
        assertEquals("Ray starts after the tangent point",
                null, sphere.findIntersections(new Ray(new Vector(2, 0, 0), new Point3D(1.5, 0, 1))));

        // **** Group: Special cases
        // TC19: Ray's line is outside, ray is orthogonal to ray start to sphere's center line
        assertEquals("Ray starts at the tangent point",
                null, sphere.findIntersections(new Ray(new Vector(0, 0, 5), new Point3D(3, 0, 0))));
