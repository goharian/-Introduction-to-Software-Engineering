...
public class GeometriesTest {
    ...
    public void findIntersections() {
        ...
        assertEquals("Error! Geometries found intersections when there were no shapes",
                null, g1.findIntsersections(new Ray(new Vector(1, 2, 3), new Point3D(4, 5, 4))));
        ...
        Geometries g2 = new Geometries(
                ...
                new Polygon(new Point3D(3, 5, 0), new Point3D(9, 11, 0), new Point3D(2, 1, 0), new Point3D(1, 0, 0)),
        ...
        assertEquals("Error! Geometries found points when there were none",
                null, g2.findIntsersections(new Ray(new Vector(0, 0, -1), new Point3D(-4, -3, -2))));
        ...
        try {
            assertEquals("Error! Geometries found invalid amount of intersections",
                    1, g2.findIntsersections(new Ray(new Vector(0, 0, -1), new Point3D(-4, -3, -2))).size());
        ...
        try {
            assertEquals("Error! Geometries didn't find all intersections",
                    g4.get_shapes().size(),
                    g4.findIntsersections(new Ray(new Vector(0, 0, 1), new Point3D(0, 0, -2))).size());
        } catch (NullPointerException e)//intersection list was null
        {
