...
public class GeometriesTest {
    ...
    void findIntsersections() {
        ...
        result = g.findIntersections(new Ray(new Vector(-5, -5, 0), new Point3D(6, 7, -0.5)));
        assertEquals("Wrong number of points", 3, result.size());


        // =============== Boundary Values Tests ==================
        // TC02: empty collection
        result = g.findIntersections(new Ray(new Vector(2, -2, -1), new Point3D(2, 2, 1)));
        ...
        g.add(new Polygon(new Point3D(0,0,0), new Point3D(4,0,0), new Point3D(4,4,0), new Point3D(0,4,0)),
                ...
                new Sphere(new Point3D(1,1,1), 2),
                new Triangle(new Point3D(0,0,0), new Point3D(4,0,0), new Point3D(0,4,0)));
        result = g.findIntersections(new Ray(new Vector(-2, -2, -1), new Point3D(-3, -3, -3)));
        assertEquals("Wrong number of points", null, result);

        //TC04: intersection with one geometry only
        result = g.findIntersections(new Ray(new Vector(-1, 1, 1), new Point3D(6, 6, 6)));
        assertEquals( "Wrong number of points",1, result.size());

        //TC05: intersection with all the geometries
        result = g.findIntersections(new Ray(new Vector(0.5, 0.8, 2.5), new Point3D(0.5, 1.2, -0.5)));
