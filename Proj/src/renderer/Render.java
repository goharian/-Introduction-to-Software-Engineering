...
public class Render {
    ...
    private Color calcColor(GeoPoint geoPoint, Ray inRay, int level, double k) {
        ...
        if (kKr > MIN_CALC_COLOR_K) {
            Ray reflectedRay = constructReflectedRay(inRay.getDirection(), n, geoPoint);
        ...
        if (kKt > MIN_CALC_COLOR_K) {
            Ray refractedRay = constructRefractedRay(inRay.getDirection(), n, geoPoint);
    ...
    private GeoPoint findClosestIntersection(Ray ray) {
        ...
        for (GeoPoint gp : intersections) {
            distance = ray.getStart().distance(gp.point);
            if (distance < min) {
                min = distance;
                closestPoint = gp;
            }
